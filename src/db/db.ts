...
const initPool = async(dbConfig) => {
    try {
        // Create a connection pool which will later be accessed via the
        // pool cache as the 'default' pool.

        if (pool === undefined || pool.status === oracledb.POOL_STATUS_CLOSED) {


            pool = await oracledb.createPool(dbConfig
                //{
                //user: /* process.env.NODE_ORACLEDB_USER || */ 'ACORREA',
            ...
            pool = oracledb.getPool(`subePool_${process.pid}`);
